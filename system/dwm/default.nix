...
with lib; let
  cfg = config.dwm.settings;
in {
  options.dwm.settings = {
    enable = mkEnableOption "enable dwm";
    enable_startx = mkEnableOption "enable startx(tty), or default lightdm";
  };
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = with pkgs; [
      ...
      st
      picom
      # inputs.picom.defaultPackage
      # inputs.picom.defaultPackage.${pkgs.system}.picom
      feh
    ...
    services = {
      xserver = {
        ...
        displayManager.startx.enable = cfg.enable_startx;
